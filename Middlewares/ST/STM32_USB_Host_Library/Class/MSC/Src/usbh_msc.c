...
static USBH_StatusTypeDef USBH_MSC_ClassRequest(USBH_HandleTypeDef *phost) {
    ...
    switch (MSC_Handle->req_state) {
        ...
        case MSC_REQ_GET_MAX_LUN:
            ...
            if (status == USBH_OK) {
                MSC_Handle->max_lun = (MSC_Handle->max_lun > MAX_SUPPORTED_LUN) ? MAX_SUPPORTED_LUN :
                                      (uint8_t) (MSC_Handle->max_lun) + 1U;
            ...
            if (MSC_Handle->current_lun < MSC_Handle->max_lun) {
                ...
                switch (MSC_Handle->unit[MSC_Handle->current_lun].state) {
                    ...
                    case MSC_READ_INQUIRY:
                        scsi_status = USBH_MSC_SCSI_Inquiry(phost, (uint8_t) MSC_Handle->current_lun,
                                                            &MSC_Handle->unit[MSC_Handle->current_lun].inquiry);
                    ...
                    case MSC_READ_CAPACITY10:
                        scsi_status = USBH_MSC_SCSI_ReadCapacity(phost, (uint8_t) MSC_Handle->current_lun,
                                                                 &MSC_Handle->unit[MSC_Handle->current_lun].capacity);
...
(int32_t) (MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_nbr *
           MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_size));
                                USBH_UsrLog ("Block number : %lu",
                                             (int32_t) (MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_nbr));
                                USBH_UsrLog ("Block Size   : %lu",
                    ...
                    case MSC_REQUEST_SENSE:
                        scsi_status = USBH_MSC_SCSI_RequestSense(phost, (uint8_t) MSC_Handle->current_lun,
                                                                 &MSC_Handle->unit[MSC_Handle->current_lun].sense);

                        if (scsi_status == USBH_OK) {
                            if ((MSC_Handle->unit[MSC_Handle->current_lun].sense.key ==
